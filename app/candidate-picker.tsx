...
export function CandidatePicker({ discovery, selectedIds, busy, error, onToggle, onGenerate, onAddCustom }: {
  ...
  return <section className="candidate-panel panel">
    ...
    <div className="notice">候选来源：AI {discovery.sources.ai === 'live' ? '建议已生成' : '使用降级候选'} · 高德 {discovery.sources.map === 'live' ? '地点已核验' : '部分待确认'} · 小红书公开检索 {discovery.sources.search === 'live' ? '已查询' : '待确认'}。AI 推荐和帖子经验均不是已验证事实。</div>
