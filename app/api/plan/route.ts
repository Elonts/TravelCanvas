...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(planStore.save(await buildPlan(discovery.request, discovery.candidates, discovery.guideSources || [], discovery.guideFoodCandidates || [])), { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
