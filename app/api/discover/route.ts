...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ error: '请求无效：请检查出发地、目的地数量、日期、预算、人数和帖子链接。' }, { status: 400 });
  try {
    return NextResponse.json(discoveryStore.save(await discoverCandidates(parsed.data)), { headers: { 'Cache-Control': 'no-store' } });
  } catch {
