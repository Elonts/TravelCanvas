...
export async function POST(request: Request) {
  ...
  try {
    const discovery = discoveryStore.get(parsed.data.discoveryId);
    ...
    return NextResponse.json(discoveryStore.append(parsed.data.discoveryId, result.candidates, result.warnings), { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
