...
export async function POST(request: Request) {
  ...
  try {
    ...
    const merged = await mergeDiscoveryWithXhsSession(current, parsed.data);
    return NextResponse.json(discoveryStore.replace(parsed.data.discoveryId, merged), { headers: { 'Cache-Control': 'no-store' } });
  } catch (cause) {
