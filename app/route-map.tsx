...
export function RouteMap({ route }: { route: RouteOverview }) {
  ...
  useEffect(() => {
    ...
    loadAmap().then(AMap => {
      ...
      for (const point of points) {
        const marker = new AMap.Marker({ position: [point.lng, point.lat], title: point.name, content: `<button class="amap-order-marker" aria-label="行程第 ${point.order} 站"><span>${point.order}</span></button>`, offset: new AMap.Pixel(-18, -36) });
  ...
  return <section className="route-map card">
    ...
    <div className="map-stage"><div ref={container} className="amap-container" aria-label="高德交互式行程路线图" />
      ...
      {active && <div className="map-popover"><div className="map-place-detail">{active.imageUrl && <img src={active.imageUrl} alt={`${active.name}地点图片`} />}<div><b><span>{active.order}</span>{active.name}</b><p>{active.city} · {active.date} · {active.time}</p><p>{active.introduction}</p><small>{active.address}<br />{incoming?.state === 'live' ? `上一站到这里：约 ${incoming.minutes} 分钟 / ${Math.round((incoming.meters || 0) / 100) / 10} 公里` : incoming ? `交通距离待确认：${incoming.error || '路线服务未返回结果'}` : '行程起点'}</small>{active.navigationUrl && <a className="nav-link" href={active.navigationUrl} target="_blank" rel="noreferrer">在高德查看 / 导航 →</a>}</div></div></div>}
    </div>
    ...
    <div className="map-legend"><span><i className="origin-dot" />出发地</span><span><i />景点 / 餐厅 / 娱乐</span><span>点击地图序号或下方地点按钮查看图片、介绍和交通距离</span></div>
    {!!route.transfers.length && <div className="transfer-list"><b>跨城衔接（按当前查询估算）</b>{route.transfers.map((leg, index) => <p key={`${leg.from}-${leg.to}-${index}`}>{leg.from} → {leg.to} · {leg.state === 'live' ? `约 ${leg.minutes} 分钟 / ${Math.round((leg.meters || 0) / 1000)} 公里` : `待确认：${leg.error || '路线未返回'}`} · {leg.transport === 'drive' ? '驾车' : '公共交通'}</p>)}</div>}
