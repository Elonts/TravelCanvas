export function attachHotelAnchors(days, hotels, orderStops) {
  return days.map(day => {
    ...
    const arrivalHotel = cityHotels.find(hotel => hotel.checkIn === day.date) || null;
    return { ...day, startHotel, endHotel, arrivalHotel, stops: orderStops(day.stops, arrivalHotel || startHotel || null) };
  });
}
