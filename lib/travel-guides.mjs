...
export async function searchTravelGuides(city, preferences = '', constraints = '', env = process.env, fetcher = fetch) {
  ...
  try {
    const data = await jsonFetch(fetcher, 'https://api.tavily.com/search', {
      ...
      body: JSON.stringify({ query: `${city} ${preferences || '综合体验'} ${constraints || ''} 旅游攻略 必去 避雷`, include_domains: ['xiaohongshu.com'], search_depth: 'advanced', chunks_per_source: 3, max_results: 8, include_answer: false, include_raw_content: false }),
    });
