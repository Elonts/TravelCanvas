...
export function createRoutePoints(origin, request, days, food) {
  ...
  days.forEach((day, dayIndex) => day.stops.forEach(stop => {
    points.push({ id: `stop:${day.date}:${stop.id}`, name: stop.name, city: day.city, date: day.date, time: stop.time, kind: stop.kind === 'entertainment' ? /** @type {const} */ ('entertainment') : /** @type {const} */ ('attraction'), address: stop.address, introduction: stop.detail, imageUrl: stop.imageUrl || null, navigationUrl: stop.navigationUrl || null, lng: stop.lng, lat: stop.lat, verified: stop.verified, poiId: stop.poiId });
