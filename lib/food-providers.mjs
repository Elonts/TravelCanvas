...
export async function searchNotes(request, stops, env = process.env, fetcher = fetch) {
  ...
  try {
    const result = await jsonFetch(fetcher, 'https://api.tavily.com/search', {
      ...
      body: JSON.stringify({ query: `${request.destinations.join(' ')} ${stops.slice(0, 6).map(s => s.name).join(' ')} ${request.foodPreferences || ''} 美食推荐榜 餐厅 旅游攻略 tips`, include_domains: ['xiaohongshu.com'], search_depth: 'basic', max_results: 8, include_answer: false, include_raw_content: false }),
    });
...
export function normalizeDiscoveryPoi(poi, city, kind, transport = 'transit') {
  ...
  const parsed = z.object({ id: z.string().min(1).max(100), name: z.string().min(2).max(200), location: z.string().max(60), address: z.union([z.string(), z.array(z.string())]).transform(value => Array.isArray(value) ? value.join('') : value), type: z.string().max(300).optional(), typecode: z.string() }).safeParse(poi);
  ...
  return {
    poiId: parsed.data.id, name: parsed.data.name, city, address: parsed.data.address, lng, lat,
  ...
  return {
    ...
    async geocode(name) {
      try {
        ...
        const row = z.object({ formatted_address: z.string().min(1), location: z.string() }).parse(result.geocodes?.[0]);
        const [lng, lat] = row.location.split(',').map(Number);
        if (!Number.isFinite(lng) || !Number.isFinite(lat) || lng < 73 || lng > 135 || lat < 18 || lat > 54) return null;
        return { name, address: row.formatted_address, lng, lat, verified: true };
      } catch { return null; }
    },
    ...
    async discover(destination, kind, names = [], limit = 4, transport = 'transit') {
      ...
      for (const result of results) if (result.status === 'fulfilled' && Array.isArray(result.value.pois)) for (const poi of result.value.pois) {
        ...
        if (candidate) unique.set(candidate.poiId, candidate);
      }
    ...
    async route(from, to, transport, city, destinationCity = city) {
      ...
      const work = (async () => {
        ...
        try {
          ...
          const result = await amap(`v3/direction/${mode}`, { origin: `${from.lng},${from.lat}`, destination: `${to.lng},${to.lat}`, ...(transport === 'transit' ? { city, cityd: destinationCity, extensions: 'base' } : {}) });
...
export async function buildFoodPlan(request, days, budget, env = process.env, fetcher = fetch, { mapIntervalMs = 400, preferredRestaurants = [], discoverySources = null } = {}) {
  ...
  for (const slot of slots) {
    ...
    if (!pools.has(key)) {
      ...
      const merged = new Map([...discovered, ...preferredRestaurants.filter(restaurant => restaurant.city === slot.city)].map(restaurant => [restaurant.id, restaurant]));
