    ...
    async route(from, to, transport, city, destinationCity = city) {
      ...
      const work = (async () => {
        ...
        try {
          ...
          return { ...base, minutes: Math.ceil(duration / 60), meters, fare, state: 'live', polyline, error: null };
...
export async function buildFoodPlan(request, days, budget, env = process.env, fetcher = fetch, { mapIntervalMs = 400, preferredRestaurants = [], discoverySources = null } = {}) {
  ...
  for (const slot of slots) {
    ...
    if (!pools.has(key)) {
      ...
      const preferred = await Promise.all(preferredRestaurants.filter(restaurant => restaurant.city === slot.city).map(async restaurant => {
  ...
  const food = { meals, sources: notes.sources, tips, warnings, searchState: notes.state, queriedAt: now(), summary: { allocated: budget.food, breakfastReserve: budget.food - Math.floor(budget.food * .8), selectedLow: 0, selectedHigh: 0, selectedCostPending: 0, extraTransport: 0, unresolved: meals.length, remaining: 0 } };
