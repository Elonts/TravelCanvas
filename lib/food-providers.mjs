import { z } from 'zod';
import { createMealSlots, evaluateRestaurant, summarizeFood } from './food.mjs';
...
export function createMapProvider(env = process.env, fetcher = fetch, { intervalMs = 400 } = {}) {
  ...
  return {
    async restaurants(slot, destination, names) {
      ...
      const params = { types: '050000', show_fields: 'business', page_size: '8' };
      const tasks = [amap('v5/place/around', { ...params, location: `${slot.previous.lng},${slot.previous.lat}`, radius: '3000', sortrule: 'distance' }),
      ...
      for (const result of results) if (result.status === 'fulfilled' && Array.isArray(result.value.pois)) {
        for (const poi of result.value.pois) { const restaurant = normalizeRestaurant(poi); if (restaurant) unique.set(restaurant.id, restaurant); }
      }
...
export async function buildFoodPlan(request, days, budget, env = process.env, fetcher = fetch, { mapIntervalMs = 400 } = {}) {
  ...
  for (const slot of slots) {
    ...
    const key = `${slot.previous.lng},${slot.previous.lat}`;
    ...
    // Evidence-bearing restaurants get a chance to compete; all still pass hard constraints.
    const candidates = attachEvidence(pool, allTips).sort((a, b) => b.tips.length - a.tips.length).slice(0, 8);
    const direct = slot.next ? await map.route(slot.previous, slot.next, request.transport, request.destination) : null;
    const options = await Promise.all(candidates.map(async restaurant => {
