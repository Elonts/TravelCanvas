...
export class DiscoveryStore {
  ...
  save(value) {
    ...
    this.entries.set(discoveryId, { value: structuredClone(value), expiresAt, version: 1 });
  ...
  get(discoveryId) {
    return this.getVersioned(discoveryId).value;
  }
  getVersioned(discoveryId) {
    this.cleanup();
    const entry = this.entries.get(discoveryId);
    if (!entry) throw Error('候选已过期，请重新发现地点');
    return { value: structuredClone(entry.value), version: entry.version, expiresAt: entry.expiresAt };
  }
  ...
  replace(discoveryId, value, expectedVersion = null) {
    this.cleanup();
    const entry = this.entries.get(discoveryId);
    if (!entry) throw Error('候选已过期，请重新发现地点');
    if (expectedVersion !== null && expectedVersion !== entry.version) throw Error('候选内容已变更，请重试');
