...
export function parseCandidatePlaces(content, fallback, limit) {
  ...
  const stops = parsed.places.map((name, i) => {
    ...
    return known ? { ...known, id: `candidate-${i}` } : {
      id: `candidate-${i}`, name, address: '地址待高德确认', lng: 0, lat: 0, verified: false,
...
export function distributeDays(stops, request) {
  ...
  const ordered = [...orderStops(candidates.filter(s => s.lng !== 0 && s.lat !== 0)), ...candidates.filter(s => s.lng === 0 || s.lat === 0)];
  let offset = 0;
