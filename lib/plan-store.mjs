...
export class PlanStore {
  ...
  change({ planId, revision, mealId, action, restaurantId }) {
    ...
    const route = entry.value.route ? { ...entry.value.route, points: createRoutePoints(entry.value.route.points.find(point => point.kind === 'origin') || null, entry.value.request, entry.value.days, food) } : null;
