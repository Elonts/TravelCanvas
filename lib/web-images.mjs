import { registerWebImage } from './web-image-store.mjs';
...
export async function fillMissingWebImages(items, env = process.env, fetcher = fetch) {
  if (!env.TAVILY_API_KEY) return items;
  const updated = [...items];
  for (let index = 0; index < updated.length; index++) {
    ...
    try {
      ...
      const raw = (json.images || []).map(image => typeof image === 'string' ? { url: image, description: '' } : image).find(image => image?.url);
      const proxyUrl = raw && registerWebImage(raw.url);
