...
export function shortlistRestaurants(restaurants, slot, request, used = new Set()) {
  ...
  const priority = restaurant => {
    ...
    return (used.has(restaurant.id) ? -1000 : 0) + (affordable ? 100 : restaurant.price ? -100 : 0)
...
export function createMealSlots(days, budget) {
  ...
  return days.flatMap((day, dayIndex) => {
    ...
    return [{ label: '午餐', index: lunchIndex, earliest: 720, latest: 840, share: .4 },
      ...
      id: `${day.date}-${i ? 'dinner' : 'lunch'}`, dayIndex, label: meal.label, date: day.date,
...
// Accept only daily schedules. Weekdays, seasonal schedules and free text remain unknown.
export function isOpenDuring(hours, start, end) {
  if (/^(全天|24小时|00:00[-–—~至]24:00)$/.test(hours.trim())) return true;
  const normalized = hours.replace(/^(每天|周一至周日|周一至周天|周一到周日|周一-周日|星期一至星期日)\s*/, '').trim();
  if (!/^(?:\d{1,2}:\d{2}\s*[-–—~至]\s*\d{1,2}:\d{2})(?:\s*[;,；、]\s*\d{1,2}:\d{2}\s*[-–—~至]\s*\d{1,2}:\d{2})*$/.test(normalized)) return null;
...
export function evaluateRestaurant(restaurant, slot, route, direct, request) {
  ...
  return { restaurant, route, direct, extraMinutes, extraFare, totalLow, totalHigh, arrival, finish,
    eligible: !reasons.length && !pending.length, reasons, pending, score,
...
export function changeMeal(food, mealId, action, restaurantId) {
  ...
  if (action === 'lock') {
    ...
    let candidates = meal.options.filter(o => o.eligible && o.restaurant.id !== meal.selectedId && !used.has(o.restaurant.id));
    ...
    if (!candidates.length) throw Error('没有符合当前预算、路线和禁忌的替代门店');
