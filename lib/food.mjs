...
export function allocateBudget(request) {
  ...
  const profiles = {
    walk: { transport: .05, stay: .38, food: .27, activities: .18 },
    transit: { transport: .12, stay: .38, food: .25, activities: .17 },
    drive: { transport: .20, stay: .35, food: .24, activities: .15 },
  };
  const profile = profiles[request.transport] || profiles.transit;
  const result = { transport: Math.floor(total * profile.transport), stay: request.days === 1 ? 0 : Math.floor(total * profile.stay), food: Math.floor(total * profile.food), activities: Math.floor(total * profile.activities), remaining: 0 };
  result.remaining = total - result.transport - result.stay - result.food - result.activities;
  return result;
}
...
export function createMealSlots(days, budget) {
  ...
  return days.flatMap((day, dayIndex) => {
    ...
    const lunchPrevious = attractions.filter(stop => toMinutes(stop.time) < 12 * 60).at(-1) || attractions[0];
    const lunchNext = day.stops.find(stop => toMinutes(stop.time) > toMinutes(lunchPrevious.time)) || null;
    const eveningAnchor = day.stops.find(stop => stop.kind === 'entertainment' && toMinutes(stop.time) >= 18 * 60) || null;
    const dinnerPrevious = attractions.filter(stop => !eveningAnchor || toMinutes(stop.time) < toMinutes(eveningAnchor.time)).at(-1) || attractions.at(-1);
    return [{ label: '午餐', previous: lunchPrevious, next: lunchNext, earliest: 720, latest: 840, share: .4 },
      { label: '晚餐', previous: dinnerPrevious, next: eveningAnchor || day.endHotel || null, earliest: eveningAnchor ? 1020 : 1080, latest: eveningAnchor ? Math.max(1020, toMinutes(eveningAnchor.time) - 60) : 1200, share: .6 }].map((meal, i) => {
      const next = meal.next;
      return {
        id: `${day.date}-${i ? 'dinner' : 'lunch'}`, dayIndex, city: day.city, label: meal.label, date: day.date,
        previous: meal.previous, next,
        earliest: meal.earliest, latest: meal.latest,
        depart: toMinutes(meal.previous.time) + (meal.previous.durationMinutes || 90),
        nextDeadline: next?.time ? toMinutes(next.time) : i && next ? 1380 : 1440,
...
export function evaluateRestaurant(restaurant, slot, route, direct, request) {
  ...
  const metersKnown = route.length === (slot.next ? 2 : 1) && route.every(r => r.state === 'live' && Number.isFinite(r.meters)) && (!slot.next || (direct?.state === 'live' && Number.isFinite(direct.meters)));
  const extraMeters = metersKnown ? Math.max(0, route.reduce((n, leg) => n + leg.meters, 0) - (direct?.meters || 0)) : null;
  ...
  const sessionHeat = restaurant.tips.filter(t => t.sourceKind === 'xhs_session').reduce((sum, tip) => sum + Math.max(0, 21 - (tip.searchRank || 20)) / 4 + Math.min(4, Math.log10((tip.visibleLikes || 0) + 1)), 0);
  const score = matches.length * 15 + Math.min(3, evidenceCount) * 5 + Math.min(10, sessionHeat) + (rankEvidence ? 3 : 0) - (extraMinutes ?? 999) * (request.foodMode === 'food' ? .3 : 2) - (totalHigh ?? 9999) / 100;
  const hardBlocked = reasons.some(reason => /饮食禁忌冲突/.test(reason));
  const canAcceptPending = !hardBlocked && (pending.length > 0 || reasons.length > 0);
  return { restaurant, route, direct, extraMinutes, extraMeters, extraFare, totalLow, totalHigh, arrival, finish,
...
export function summarizeFood(food) {
  ...
  const knownLow = chosen.reduce((n, o) => n + (o.totalLow ?? 0), 0);
  const knownHigh = chosen.reduce((n, o) => n + (o.totalHigh ?? 0), 0);
  return { ...food.summary, selectedLow: knownLow, selectedHigh: knownHigh, selectedCostPending: chosen.filter(o => o.totalHigh === null).length,
    extraTransport: chosen.reduce((n, o) => n + (o.extraFare ?? 0), 0),
    unresolved: food.meals.length - chosen.length,
    remaining: food.summary.allocated - food.summary.breakfastReserve - knownHigh };
}

export function asDraftFood(food) {
  const draft = structuredClone(food);
  draft.meals = draft.meals.map(meal => ({ ...meal, draftSelectedId: meal.selectedId, selectedId: null, locked: false }));
  const preview = { ...draft, meals: draft.meals.map(meal => ({ ...meal, selectedId: meal.draftSelectedId || null })) };
  draft.summary = summarizeFood(preview);
  return draft;
}

export function changeDraftMeal(food, mealId, action, restaurantId) {
  let working = structuredClone(food);
  working.meals = working.meals.map(meal => ({ ...meal, selectedId: meal.draftSelectedId || null }));
  if (action === 'skip') {
    const meal = working.meals.find(item => item.slot.id === mealId);
    if (!meal) throw Error('餐次不存在');
    meal.selectedId = null;
  } else if (action === 'move') {
    const meal = working.meals.find(item => item.slot.id === mealId);
    if (!meal?.selectedId) throw Error('当前餐次没有可调整的餐厅');
    const restaurant = meal.selectedId;
    const alternatives = working.meals.filter(item => item.slot.id !== mealId).flatMap(item => item.options
      .filter(option => option.restaurant.id === restaurant && !option.hardBlocked)
      .map(option => ({ meal: item, option })))
    ...
    if (!alternatives.length) throw Error('没有找到更接近的其他日期或餐次，请查看附近同类餐厅');
    meal.selectedId = null;
    alternatives[0].meal.selectedId = restaurant;
  } else working = changeMeal(working, mealId, action, restaurantId);
  working.meals = working.meals.map(meal => ({ ...meal, draftSelectedId: meal.selectedId, selectedId: null, locked: false }));
  const preview = { ...working, meals: working.meals.map(meal => ({ ...meal, selectedId: meal.draftSelectedId || null })) };
  working.summary = summarizeFood(preview);
  return working;
}
...
export function changeMeal(food, mealId, action, restaurantId) {
  ...
  if (action === 'lock') {
    ...
    let candidates = meal.options.filter(o => (o.eligible || (['select', 'selectAndLock'].includes(action) && o.canAcceptPending)) && o.restaurant.id !== meal.selectedId && !used.has(o.restaurant.id));
    ...
    else if (['select', 'selectAndLock'].includes(action)) candidates = candidates.filter(o => o.restaurant.id === restaurantId);
    ...
    if (action === 'selectAndLock') meal.locked = true;
  }
