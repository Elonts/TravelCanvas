...
export const changeSchema = z.object({ planId: z.string().uuid(), revision: z.number().int().nonnegative(), mealId: z.string().min(1).max(80), action: z.enum(['lock', 'cheaper', 'closer', 'select', 'selectAndLock']), restaurantId: z.string().min(1).max(100).optional() });
export const restaurantSearchSchema = z.object({ planId: z.string().uuid(), revision: z.number().int().nonnegative(), mealId: z.string().min(1).max(80), names: z.array(z.string().trim().min(2).max(100)).min(1).max(8) });
