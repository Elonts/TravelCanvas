...
const restaurantRequestBase = { planId: z.string().uuid(), revision: z.number().int().nonnegative() };
export const restaurantSearchSchema = z.union([
  z.object({ ...restaurantRequestBase, mealId: z.string().min(1).max(80).optional(), names: z.array(z.string().trim().min(2).max(100)).min(1).max(8) }).strict(),
  z.object({ ...restaurantRequestBase, manualInput: z.string().trim().min(2).max(100), restaurantId: z.string().min(1).max(120), mealId: z.string().min(1).max(80) }).strict(),
]);
...
export const dayReplanSchema = z.object({
  ...
  entertainmentSelections: z.array(z.object({ id: z.string().min(1).max(200), anchorPointId: z.string().min(1).max(240), position: z.enum(['before', 'after']) })).max(3)
...
export const entertainmentSearchSchema = z.object({
  ...
  anchorPointId: z.string().min(1).max(240), position: z.enum(['before', 'after']),
