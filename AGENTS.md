...
- 真实地点图片优先使用高德 POI 等可追溯 Provider 返回的图片，并经服务端受控代理提供；缺图使用明确的类别占位图。不得用 AI 生成图片冒充真实景区、饭店或娱乐场所。
- 美食发现只通过 Tavily 搜索公开收录的小红书笔记；AI 仅从搜索摘要或用户提供正文中提取逐字可定位的店名、分店和证据，不得声称覆盖小红书站内全部内容，也不得把搜索摘要、榜单线索或 AI 判断包装成官方认证。
