...
export async function fixtureFetch(input, options = {}) {
  ...
  if (url.hostname === 'restapi.amap.com') {
    ...
    if (url.pathname.includes('/place/')) {
      ...
      if (url.pathname.includes('/detail') && ['station-shanghai', 'station-hangzhou'].includes(url.searchParams.get('id'))) return reply({ status: '1', pois: [{ id: url.searchParams.get('id'), name: url.searchParams.get('id') === 'station-shanghai' ? '上海虹桥站' : '杭州东站', address: '测试车站地址', location: url.searchParams.get('id') === 'station-shanghai' ? '121.32,31.19' : '120.21,30.29', typecode: '150200' }] });
      ...
      if (url.searchParams.get('types') === '150200') return reply({ status: '1', pois: [{ id: 'station-hangzhou', name: '杭州东站', address: '测试车站地址', location: '120.21,30.29', typecode: '150200' }] });
